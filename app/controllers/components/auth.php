 ...
 */
    ...
    function identifyAll($data) {
        ...
        $models = array('Client','User');
        foreach ($models as $model) {
            $this->userModel     = $model;
            if($model == 'User'){
                $this->userScope     = array('User.status' => 1);
                $this->loginAction   = array('prefix' => 'staff', 'staff' => true, 'controller' => 'users', 'action' => 'alllogin');
                $this->loginRedirect = array('prefix' => 'staff', 'staff' => true, 'controller' => 'users', 'action' => 'alllogin');
            }else{
                $this->userScope     = array('Client.status' => 1);
                $this->loginAction   = array('prefix' => 'client', 'client' => false, 'controller' => 'clients', 'action' => 'login');
                $this->loginRedirect = array('prefix' => 'client', 'client' => false, 'controller' => 'clients', 'action' => 'index');
            }
            
            $dataNew[$model.'.username'] = $data['User.username'];
            $dataNew[$model.'.password'] = $data['User.password'];
           //unset($data['User.username']); unset($data['User.password']);
             //echo '<pre>'; print_r($dataNew);
            $result = $this->identify($dataNew); // let cake do its thing
            //print_r($result);
            if ($result) {
                $this->sessionKey = 'Auth.'.$model;
	...
	function login($data = null) {
		...
		if (empty($data)) {
			$data = $this->data;
                ...
                if($this->sessionKey == 'Auth.User'){
                    if ($user = $this->identifyAll($data)) {
 ...
 */
	function hashPasswords($data) {
		if (is_object($this->authenticate) && method_exists($this->authenticate, 'hashPasswords')) {
			//debug($this->authenticate->hashPasswords($data));exit;
