...
function get_adr_pickup_chart(){
        ...
        $.post( 
             "/clients/get_adr_pickup_chart/"+client_id+"/"+pickup_date+"/"+month+"/"+year,
             { name: "Zara" },
             function(data) {
...
function get_chart(client_id){
   ...
   $('#chart_area').html('<div style="text-align:center;">Loading Please wait....</div>');
    ...
    $.post( 
         "/clients/get_chart/"+client_id+"/"+hotel_month+"/"+hotel_year+"/"+departmentId,
         { name: "Zara" },
         function(data) {
...
function get_fcst_chart(client_id){
    ...
    $("#hotel_month").attr("onChange","get_fcst_chart($('#hotel_id').val());");
       ...
       $('#chart_area').html('<div style="text-align:center;">Loading Please wait....</div>');
        $('#new_graph_chart').remove();
        $.post( 
             "/clients/get_forecast_chart/"+client_id+"/"+hotel_month+"/"+hotel_year,
             { name: "Zara" },
             function(data) {
                var data_new = '<div id="new_graph_chart">'+data+'</div>';
                $('#chart_area').html(data_new);
...
function get_combined_chart(client_id){
        ...
        $('#new_graph_chart').remove();
        $.post( 
             "/clients/get_combined_chart/"+client_id+"/"+hotel_month+"/"+hotel_year,
             { name: "Zara" },
             function(data) {
                var data_new = '<div id="new_graph_chart">'+data+'</div>';
                $('#chart_area').html(data_new);
...
<input type="hidden" id="clientId" value="<?php echo $clientId; ?>"></input>
...
<input type="button" onClick="get_pickup_chart();" value="GO" class="btn btn-info btn-small">
        <br/>
    ...
    <br/><br/>
     ...
     <div class="widget-box">
        ...
        <input type="button" onClick="get_adr_pickup_chart();" value="GO" class="btn btn-info btn-small">
        <br/>
...
<tr>
<td width="50%">Total Departments</td>
<td width="50%"><?php echo $deparmentCount?></td>
</tr>
<tr>
<td width="50%">Total Users</td>
<td width="50%"><?php echo $usersCount ?></td>
</tr>
<tr>
<td width="50%">Users Assigned on Departments</td>
<td width="50%"><?php echo $assignuserCount ?></td>
</tr>
</table>-->
...
$('#hotel_id option[value="'+clientId+'"]').prop('selected', true);
$('#hotel_id').trigger('change');

$('#weekly_report_hotel option[value="'+clientId+'"]').prop('selected', true);
//$('#weekly_report_hotel > :nth-child(2)').prop('selected', true);
$('#weekly_report_hotel').trigger('change');

$('#pickup_hotel_id option[value="'+clientId+'"]').prop('selected', true);
