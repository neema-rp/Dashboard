...
<script>
   ...
   function get_pickup_chart(){
        ...
        $.post( 
             "/admins/get_pickup_chart_new/"+client_id+"/"+pickup_date+"/"+month+"/"+year,
             { name: "Zara" },
             function(data) {
...
function get_adr_pickup_chart(){
        ...
        $.post( 
             "/admins/get_adr_pickup_chart/"+client_id+"/"+pickup_date+"/"+month+"/"+year,
             { name: "Zara" },
             function(data) {
...
function view_report(client_id){
    var report_link = '<a href="/admin/sheets/weekly_report/'+client_id+'" target="_blank"><button class="btn btn-info">View Report<i class="icon-print  bigger-125 icon-on-right"></i></button></a>';
    $('#report_link_div').html(report_link);
    
    $('#hotel_id option[value="'+client_id+'"]').attr('selected', true);
    $('#pickup_hotel_id option[value="'+client_id+'"]').attr('selected', true);
    $('#pickup_adr_hotel_id option[value="'+client_id+'"]').attr('selected', true);
    
    get_chart(client_id);
    $('#pickup_hotel_id option[value="'+client_id+'"]').prop('selected', true);
    $('#pickup_adr_hotel_id option[value="'+client_id+'"]').prop('selected', true);
    get_pickup_chart();
...
//        $("#departmentId").attr("onChange","get_chart($('#hotel_id').val());");

       $('#chart_area').html('<div style="text-align:center;">Loading Please wait....</div>');
        $('#new_graph_chart').remove();
        $.post( 
             "/admins/get_chart/"+client_id+"/"+hotel_month+"/"+hotel_year+"/"+departmentId,
             { name: "Zara" },
             function(data) {
        ...
        <div class="widget-box" style="display: none;">
                ...
                <div class="widget-body">
                 <div class="widget-main">
                    <div class="row-fluid">
                            <select name="weekly_report_hotel" id="weekly_report_hotel" class="chzn-select" onChange="view_report(this.value);" data-placeholder="Choose a Hotel">
                            <option>Select Hotel</option>
                            <?php foreach($all_hotels as $chld){ ?>
        ...
        <div class="widget-box">
                ...
                <div class="widget-body">
                 <div class="widget-main">
                    <div class="row-fluid">
                        <div style="width: 100%;">
                            <select name="hotel_id" id="hotel_id"  class="chzn-select" onChange="$('#departmentId').html('');get_chart(this.value);">
                                <option>Select Hotel</option>
                                <?php foreach($all_hotels as $chld){ ?>
                                    <option value="<?php echo $chld['Client']['id']; ?>"><?php echo $chld['Client']['hotelname']; ?></option>            
                                <?php } ?>
                           ...
                           <select name="hotel_month" class="chzn-select" id="hotel_month" onChange="get_chart($('#hotel_id').val());">
                        ...
                        <select name="hotel_year"  class="chzn-select" id="hotel_year" onChange="get_chart($('#hotel_id').val());">
       ...
       <div class="widget-box">
                ...
                <div class="widget-body">
                 <div class="widget-main">
                    <div class="row-fluid">
                        <div style="width: 100%;">
                            <select name="hotel_id" id="pickup_hotel_id" class="chzn-select">
                                <option>Select Hotel</option>
                                <?php foreach($all_hotels as $chld){ ?>
                                <option value="<?php echo $chld['Client']['id']; ?>"><?php echo $chld['Client']['hotelname']; ?></option>            
                                <?php } ?>
                            ...
                            <select style="border:1px solid #ccc;" name="hotel_month_pickup" id="hotel_month_pickup" class="chzn-select">
                                <option value="0">Select Month</option>
                            ...
                            <select name="hotel_year_pickup" id="hotel_year_pickup" class="chzn-select">
                                <option value="0">Select Year</option>
                                <option value="<?php echo date('Y'); ?>" selected="selected"><?php echo date('Y'); ?></option>            
                                <option value="<?php echo date('Y',strtotime('+1 Year')); ?>"><?php echo date('Y',strtotime('+1 Year')); ?></option>
                           </select>

                            <select name="pickup" id="pickup" class="chzn-select">
                            ...
                            <button class="btn btn-info btn-small" onClick="get_pickup_chart();">GO</button>
                        </div>
                 ...
                 <div class="widget-main">
                    <div class="row-fluid">
                        <div style="width: 100%;">
                            ...
                            <select name="pickup_adr_hotel_id" id="pickup_adr_hotel_id" class="chzn-select">
                                    <option>Select Hotel</option>
                            <?php foreach($all_hotels as $chld){ ?>
                                 <option value="<?php echo $chld['Client']['id']; ?>"><?php echo $chld['Client']['hotelname']; ?></option>            
                            <?php } ?>
                            ...
                            <select name="adr_hotel_month_pickup" id="adr_hotel_month_pickup" class="chzn-select">
                                <option value="0">Select Month</option>
                                <option value="<?php echo date('m',strtotime(date('m/1/Y').' -2 Month')); ?>"><?php echo date('F',strtotime(date('m/1/Y').' -2 Month')); ?></option>
                                <option value="<?php echo date('m',strtotime(date('m/1/Y').' -1 Month')); ?>"><?php echo date('F',strtotime(date('m/1/Y').' -1 Month')); ?></option>
                                <option value="<?php echo date('m'); ?>" selected="selected"><?php echo date('F'); ?></option>            
                            ...
                            <select name="adr_hotel_year_pickup" id="adr_hotel_year_pickup" class="chzn-select">
                                <option value="0">Select Year</option>
                                <option value="<?php echo date('Y'); ?>" selected="selected"><?php echo date('Y'); ?></option>            
                                <option value="<?php echo date('Y',strtotime('+1 Year')); ?>"><?php echo date('Y',strtotime('+1 Year')); ?></option>
                           </select>

                            <select name="adr_pickup" id="adr_pickup" class="chzn-select" >
                                    <option value="0">Pickup Date</option>
                                    <?php for($day=1;$day <= 31; $day++){ ?>
                                         <option value="<?php echo $day; ?>" <?php if(date('d',strtotime('-1 Day')) == $day){ echo 'selected'; } ?>><?php echo $day; ?></option>            
                                    <?php } ?>
                            ...
                            <button class="btn btn-info btn-small" onClick="get_adr_pickup_chart();">GO</button>
                        </div>
