...
for($month=$financial_month_start;$month <= ($financial_month_start + 11); $month++){ 
    if($month > '12'){
        $useYearArr[$month] = $year+'1';
         ...
         if(array_search($useMonthArr[$month], $month_arr) < array_search($gps_month, $month_arr)){
            $useYearArr[$month] = $year-'1';
         }
     }
}
echo '<pre>'; print_r($useMonthArr); print_r($useYearArr); exit;
