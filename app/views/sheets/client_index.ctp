...
<div class="control-group">
	...
	<?php echo $this->Form->create('Sheet', array('url' => array('prefix' => 'client', 'client' => true, 'controller' => 'sheets', 'action' => 'index'))); ?>
	...
	<tr<?php echo $class;?>>
		...
		if($sheet['Sheet']['status'] == 0)
		{
		  $lockaction = 'unlock';
		  $lockname = 'Unlock';
		  echo $this->Html->link(__('<i class="icon-qrcode bigger-130"></i>', true), '#',array('onclick'=>'alert("This sheet is locked.")','title' => 'Sheets', 'escape' => false,'class'=>'blue'));
		}
		else
		{
		  $lockaction = 'lock';
		  $lockname = 'Lock';
                  echo $this->Html->link(__('<i class="icon-qrcode bigger-130"></i>', true), array('prefix' => 'client', 'client' => true, 'action' => 'webform', $sheet['Sheet']['id']),array('title' => 'Sheets', 'escape' => false,'class'=>'blue'));
                  echo " | ";
                  echo $this->Html->link(__('<i class="icon-download-alt bigger-130"></i>', true), array('prefix' => 'client', 'client' => true, 'action' => 'pdfwebforms', $sheet['Sheet']['id']),array('target'=>'_blank','title' => 'Archive', 'escape' => false,'class'=>'green')); 
		}
                echo " | ";
                echo $this->Html->link(__('Copy Col.', true), array('action' => 'copy_column', $sheet['Sheet']['id'])); 
                echo " | ";
                echo $this->Html->link(__('<i class="icon-zoom-in bigger-130"></i>', true), array('action' => 'view', $sheet['Sheet']['id']), array('title' => 'View', 'escape' => false,'class'=>'blue'));
                echo " | "; 
                echo $this->Html->link(
                            $lockname,
