...
<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.4.2/jquery.min.js"></script>
    ...
    <script type="text/javascript">
	  ...
	  window.location = 'http://<?php echo $_SERVER['HTTP_HOST'].'/staff/sheets/viewchart/'.$this->params['pass'][0].'/'.$this->params['pass'][1].'/'; ?>'+ $("#year").val();
	});

      $("#column").change(function(){
	      var year = $("#year").val()
	      if(year == ""){
		window.location = 'http://<?php echo $_SERVER['HTTP_HOST'].'/staff/sheets/viewchart/'.$this->params['pass'][0].'/'.$this->params['pass'][1].'/year/'; ?>'+ $("#column").val();
	      }else{
		  window.location = 'http://<?php echo $_SERVER['HTTP_HOST'].'/staff/sheets/viewchart/'.$this->params['pass'][0].'/'.$this->params['pass'][1].'/'; ?>'+year + '/'+ $("#column").val();
	      }
	});
...
//alert("sfsdfdf "+$("#month").val());
	      ...
	      var year = $("#year").val()
	      if(year == ""){
		window.location = 'http://<?php echo $_SERVER['HTTP_HOST'].'/staff/sheets/viewchart/'.$this->params['pass'][0].'/'.$this->params['pass'][1].'/year/'; ?>'+ $("#column").val()+'/'+$("#month").val();
	      }else{
		  window.location = 'http://<?php echo $_SERVER['HTTP_HOST'].'/staff/sheets/viewchart/'.$this->params['pass'][0].'/'.$this->params['pass'][1].'/'; ?>'+year + '/'+ $("#column").val()+'/'+$("#month").val();
	      }
	});
...
<?php /*<option value="2010" <?php if(isset($year) && $year == "2010"){ echo "selected = 2010"; } ?>>2010</option>
